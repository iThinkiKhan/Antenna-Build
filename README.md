_Update 2: I ended up moving the to the attic with my OrangePi 02W a local node for the RTL SDR. This has the dual advantage of not having to run cable, and keeping the signal crisp and clear (no cable degradation). It has a case that doubles as an aluminum heatsink and external 2.4Ghz antenna, which makes it ideal for this project. Im currently getting excellent wifi reception from the attic (-44 dbm), plenty sufficient for streaming the live data. The OrangePi is also setup as my home network sidecar, to restart things if the main lab ever loses power. My attic is not lined with foil insulation, so my signal up there is excellent. I have to be careful not to clip out the RTL SDR. Early testing and some AI math shows my gain at about 700x by moving it up. Its also completely hidden and all the data is processed locally or streamed over the air. Sweet!_
